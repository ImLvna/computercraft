config = {
  ...
  password = "dsjaudhasiyudgasy7idasgu",

  connectionURL = "ws://localhost:8081/node",
    ...
    EVAL = function(packet)
        ...
        if not func then
            ...
            if not result[1] then
                socket.send(textutils.serialiseJSON({ type = "EVAL", nonce = packet.nonce, data = {success = false, output = result[2] }}))
            else
                table.remove(result, 1)
                socket.send(textutils.serialiseJSON({ type = "EVAL", nonce = packet.nonce, data = {success = true, output = result[1]}}))
            end
        end
    end,
}
